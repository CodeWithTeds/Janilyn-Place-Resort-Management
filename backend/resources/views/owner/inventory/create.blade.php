                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="category" name="category" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="Linen" {{ old('category') == 'Linen' ? 'selected' : '' }}>Linen (Sheets, Towels, etc.)</option>
                                <option value="Toiletries" {{ old('category') == 'Toiletries' ? 'selected' : '' }}>Toiletries (Soap, Shampoo, etc.)</option>
                                <option value="Cleaning Supplies" {{ old('category') == 'Cleaning Supplies' ? 'selected' : '' }}>Cleaning Supplies</option>
                                <option value="Minibar" {{ old('category') == 'Minibar' ? 'selected' : '' }}>Minibar Items</option>
                        ...
                        <div>
                            <x-label for="quantity" value="{{ __('Initial Quantity') }}" />
                            <x-input id="quantity" class="block mt-1 w-full" type="number" name="quantity" :value="old('quantity', 0)" required min="0" />
                            <x-input-error for="quantity" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="unit" value="{{ __('Unit of Measurement') }}" />
                            <x-input id="unit" class="block mt-1 w-full" type="text" name="unit" :value="old('unit', 'pcs')" required placeholder="pcs, box, set, etc." />
                            <x-input-error for="unit" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="reorder_level" value="{{ __('Low Stock Alert Level') }}" />
                            <x-input id="reorder_level" class="block mt-1 w-full" type="number" name="reorder_level" :value="old('reorder_level', 10)" required min="0" />
                        ...
                        <div>
                            ...
                            <x-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location')" placeholder="e.g. Storage Room A" />
                            <x-input-error for="location" class="mt-2" />
                        </div>
                        
                        <div>
                            <x-label for="cost_per_unit" value="{{ __('Cost Per Unit (Optional)') }}" />
                            <x-input id="cost_per_unit" class="block mt-1 w-full" type="number" step="0.01" name="cost_per_unit" :value="old('cost_per_unit')" min="0" />
