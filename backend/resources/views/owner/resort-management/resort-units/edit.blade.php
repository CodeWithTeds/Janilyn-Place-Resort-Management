    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form action="{{ route('resort-management.resort-units.update', $resortUnit->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <x-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $resortUnit->name)" required autofocus />
                        ...
                        <div>
                            ...
                            <select id="room_type_id" name="room_type_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                ...
                                @foreach($roomTypes as $type)
                                    <option value="{{ $type->id }}" {{ old('room_type_id', $resortUnit->room_type_id) == $type->id ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                <option value="available" {{ old('status', $resortUnit->status) == 'available' ? 'selected' : '' }}>Available</option>
                                <option value="maintenance" {{ old('status', $resortUnit->status) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                                <option value="occupied" {{ old('status', $resortUnit->status) == 'occupied' ? 'selected' : '' }}>Occupied</option>
                            </select>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea id="notes" name="notes" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" rows="3">{{ old('notes', $resortUnit->notes) }}</textarea>
                            <x-input-error for="notes" class="mt-2" />
                        </div>

                        <!-- Unit-Specific Pricing Tiers -->
                        <div class="col-span-1 md:col-span-2 mt-6">
                            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200" x-data="{ tiers: {{ $resortUnit->pricingTiers->toJson() }} }">
                                ...
                                <p class="text-sm text-gray-500 mb-4">Add pricing tiers specific to this unit. These will override room type pricing tiers.</p>
                                    ...
                                    <template x-for="(tier, index) in tiers" :key="index">
                                        <div class="border rounded-lg bg-white overflow-hidden shadow-sm" x-data="{ expanded: false }">
                                            ...
                                            <div x-show="expanded" x-collapse class="p-4 border-t border-gray-200 bg-gray-50 grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <!-- Hidden fields for association logic in controller if needed, but usually controller handles it based on context -->
