                ...
                <form action="{{ route('owner.resort-management.room-types.update', $roomType) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <x-input id="category" class="block mt-1 w-full" type="text" name="category" :value="old('category', $roomType->category)" />
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            @if($roomType->image_path)
                                <div class="mt-2 mb-4">
                                    <img src="{{ Storage::url($roomType->image_path) }}" alt="{{ $roomType->name }}" class="w-32 h-32 object-cover rounded-md">
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <x-input id="base_price_weekday" class="block mt-1 w-full" type="number" step="0.01" name="base_price_weekday" :value="old('base_price_weekday', $roomType->base_price_weekday)" required />
                            <x-input-error for="base_price_weekday" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="base_price_weekend" value="{{ __('Base Price (Weekend)') }}" />
                            <x-input id="base_price_weekend" class="block mt-1 w-full" type="number" step="0.01" name="base_price_weekend" :value="old('base_price_weekend', $roomType->base_price_weekend)" required />
                            <x-input-error for="base_price_weekend" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="extra_person_charge" value="{{ __('Extra Person Charge') }}" />
                            <x-input id="extra_person_charge" class="block mt-1 w-full" type="number" step="0.01" name="extra_person_charge" :value="old('extra_person_charge', $roomType->extra_person_charge)" />
                            <x-input-error for="extra_person_charge" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="cooking_fee" value="{{ __('Cooking Fee') }}" />
                            <x-input id="cooking_fee" class="block mt-1 w-full" type="number" step="0.01" name="cooking_fee" :value="old('cooking_fee', $roomType->cooking_fee)" />
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea id="amenities" name="amenities" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" rows="2">{{ old('amenities', $roomType->amenities) }}</textarea>
