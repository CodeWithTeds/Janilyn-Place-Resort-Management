                                ...
                                @if($roomType->image_path)
                                    <div class="mt-2" x-show="!photoPreview">
                                        <div class="relative w-full h-56 rounded-lg overflow-hidden mb-4 border border-gray-200 shadow-sm group">
                                            <img src="{{ asset('storage/' . $roomType->image_path) }}" alt="{{ $roomType->name }}" class="w-full h-full object-cover">
                                            <div class="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                <button type="button" @click.prevent="document.getElementById('image').click()" class="bg-white text-indigo-600 rounded-full p-2 hover:bg-indigo-50 transition-colors duration-200 shadow-lg font-medium px-4">
                                                    Change Photo
                                                </button>
                                            </div>
                                        </div>
                                        <div class="text-center">
                                            <button type="button" @click.prevent="document.getElementById('image').click()" class="text-sm text-indigo-600 hover:text-indigo-800 font-medium">Click to replace photo</button>
                                        </div>
                                    </div>
                                @else
                            ...
                            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 max-w-md mx-auto">
                                ...
                                <div class="space-y-5">
                                    <div>
                                        ...
                                        <x-input id="name" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200" type="text" name="name" :value="old('name', $roomType->name)" required autofocus placeholder="e.g. Ocean View Suite" />
                                    ...
                                    <div>
                                        ...
                                        <select id="category" name="category" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200 bg-white">
                                            ...
                                            <option value="DELUXE ROOM" {{ old('category', $roomType->category) == 'DELUXE ROOM' ? 'selected' : '' }} class="font-medium">🌟 DELUXE ROOM</option>
                                            <option value="GUEST HOUSE" {{ old('category', $roomType->category) == 'GUEST HOUSE' ? 'selected' : '' }} class="font-medium">🏠 GUEST HOUSE</option>
                                            <option value="APARTMENT STYLE" {{ old('category', $roomType->category) == 'APARTMENT STYLE' ? 'selected' : '' }} class="font-medium">🏢 APARTMENT STYLE</option>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <textarea id="description" name="description" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm block mt-2 w-full transition-colors duration-200" rows="4" placeholder="Describe the room features, view, and amenities...">{{ old('description', $roomType->description) }}</textarea>
                                        <x-input-error for="description" class="mt-2" />
                                    </div>

                                    <div class="bg-indigo-50 p-3 rounded-lg border border-indigo-100">
                                        <label for="is_package" class="flex items-center cursor-pointer">
                                            <x-checkbox id="is_package" name="is_package" value="1" :checked="old('is_package', $roomType->is_package) == 1" class="text-indigo-600 focus:ring-indigo-500" />
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        <x-label for="min_pax" value="{{ __('Minimum Guests') }}" class="font-medium text-gray-700" />
                                        <x-input id="min_pax" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200" type="number" name="min_pax" :value="old('min_pax', $roomType->min_pax)" required min="1" placeholder="2" />
                                        <x-input-error for="min_pax" class="mt-2" />
                                    </div>
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        <x-label for="max_pax" value="{{ __('Maximum Guests') }}" class="font-medium text-gray-700" />
                                        <x-input id="max_pax" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200" type="number" name="max_pax" :value="old('max_pax', $roomType->max_pax)" required min="1" placeholder="4" />
                                        <x-input-error for="max_pax" class="mt-2" />
                                    </div>
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        <x-label for="bedroom_count" value="{{ __('Bedrooms') }}" class="font-medium text-gray-700" />
                                        <x-input id="bedroom_count" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200" type="number" name="bedroom_count" :value="old('bedroom_count', $roomType->bedroom_count)" min="0" placeholder="1" />
                                        <x-input-error for="bedroom_count" class="mt-2" />
                                    </div>
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        <x-label for="max_day_guests" value="{{ __('Day Visitors') }}" class="font-medium text-gray-700" />
                                        <x-input id="max_day_guests" class="block mt-2 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm transition-colors duration-200" type="number" name="max_day_guests" :value="old('max_day_guests', $roomType->max_day_guests)" min="0" placeholder="6" />
                                        <p class="text-xs text-gray-500 mt-1">Maximum guests allowed until 10pm</p>
                            ...
                            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6" x-data="{ 
                                    ...
                                    <div class="md:col-span-2 bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-xl border border-indigo-200">
                                    ...
                                    <div>
                                        <x-label for="base_price_weekday" x-text="samePrice ? '{{ __('Base Price / Room Rate') }}' : '{{ __('Weekday Price') }}'" class="font-medium text-gray-700" />
                                        <div class="relative mt-2 rounded-lg shadow-sm bg-gray-50">
                                            ...
                                            <input id="base_price_weekday" type="number" step="0.01" 
                                                ...
                                                @input="updatePrices()"
                                                class="block w-full rounded-lg border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50 transition-colors duration-200" 
                                    ...
                                    <div x-show="!samePrice" style="display: none;">
                                        <x-label for="base_price_weekend" value="{{ __('Weekend Price') }}" class="font-medium text-gray-700" />
                                        <div class="relative mt-2 rounded-lg shadow-sm bg-gray-50">
                                            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                                                <span class="text-gray-700 font-bold">₱</span>
                                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                                    <div>
                                        ...
                                        <div class="relative mt-2 rounded-lg shadow-sm bg-gray-50">
                                            ...
                                            <input id="extra_person_charge" type="number" step="0.01" name="extra_person_charge" value="{{ old('extra_person_charge', $roomType->extra_person_charge) }}" class="block w-full rounded-lg border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50 transition-colors duration-200" required min="0" />
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative mt-2 rounded-lg shadow-sm bg-gray-50">
                                            ...
                                            <input id="cooking_fee" type="number" step="0.01" name="cooking_fee" value="{{ old('cooking_fee', $roomType->cooking_fee) }}" class="block w-full rounded-lg border-gray-300 pl-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm bg-gray-50 transition-colors duration-200" required min="0" />
                                        </div>
                            ...
                            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <textarea id="amenities" name="amenities" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm block mt-2 w-full transition-colors duration-200" rows="4" placeholder="WiFi, Air Conditioning, Smart TV, Hot Shower, Kitchenette, Ocean View, Private Balcony">{{ old('amenities', $roomType->amenities) }}</textarea>
