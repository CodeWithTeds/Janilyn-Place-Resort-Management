    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form action="{{ route('owner.resort-management.room-types.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label for="is_package" class="flex items-center">
                                <x-checkbox id="is_package" name="is_package" value="1" :checked="old('is_package') == 1" />
                        ...
                        <div>
                            ...
                            <x-input id="bedroom_count" class="block mt-1 w-full" type="number" name="bedroom_count" :value="old('bedroom_count', 0)" min="0" />
                            <x-input-error for="bedroom_count" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="max_day_guests" value="{{ __('Max Day Guests (until 10pm)') }}" />
                            <x-input id="max_day_guests" class="block mt-1 w-full" type="number" name="max_day_guests" :value="old('max_day_guests', 0)" min="0" />
                        ...
                        <div>
                            ...
                            <x-input id="extra_person_charge" class="block mt-1 w-full" type="number" step="0.01" name="extra_person_charge" :value="old('extra_person_charge', 0)" required min="0" />
                            <x-input-error for="extra_person_charge" class="mt-2" />
                        </div>

                        <div>
                            <x-label for="cooking_fee" value="{{ __('Cooking Fee') }}" />
                            <x-input id="cooking_fee" class="block mt-1 w-full" type="number" step="0.01" name="cooking_fee" :value="old('cooking_fee', 0)" required min="0" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="amenities" name="amenities" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" rows="3" placeholder="WiFi, AC, TV, etc.">{{ old('amenities') }}</textarea>
