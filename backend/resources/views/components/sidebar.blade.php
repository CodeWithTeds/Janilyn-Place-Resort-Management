    ...
    <div class="flex flex-col flex-1 overflow-y-auto">
        <nav class="flex-1 px-4 py-4 space-y-2">
            ...
            @if(Auth::check() && (Auth::user()->isOwner() || Auth::user()->isStaff()))
                ...
                <div x-data="{ open: {{ request()->routeIs('resort-management.guest-management.*') ? 'true' : 'false' }} }">
                    <button @click="open = !open" class="flex items-center w-full px-4 py-2 text-brand-100 hover:bg-brand-700 hover:text-white rounded-md group focus:outline-none transition ease-in-out duration-150">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div x-show="open" class="mt-1 space-y-1 pl-4" x-cloak>
                        <x-sidebar-link href="{{ route('resort-management.guest-management.index') }}" :active="request()->routeIs('resort-management.guest-management.index')" class="text-sm">
                            {{ __('Guest History') }}
                        </x-sidebar-link>
                        <x-sidebar-link href="{{ route('resort-management.guest-management.loyalty') }}" :active="request()->routeIs('resort-management.guest-management.loyalty')" class="text-sm">
                            {{ __('Loyalty & Rewards') }}
                        </x-sidebar-link>
                    </div>
                </div>
